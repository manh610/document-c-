...
class Dothi{
	...
	public:
		...
		int		d[MAX], dFloyd[MAX][MAX];			// d: mang khoang cach tu 1 dinh den cac dinh khac; dFloyd: k/c giua cac cap dinh
		...
		void	Dijkstra(int s);					// Ham tim duong di ngan nhat tu dinh s den cac dinh con lai cua do thi co huong, trong so duong
		void	Bellman_Ford(int s);				// Ham tim duong di ngan nhat tu dinh s den cac dinh con lai cua do thi co huong khong co chu trinh am
		void	Floyd();							// Ham tim duong di ngan nhat giua cac cap dinh cua do thi co huong khong co chu trinh am
		Dothi();									// Contructor
...
// Ham tim duong di ngan nhat tu 1 dinh s den cac dinh khac cua do thi (theo thuat toan Dijkstra)
void	Dothi::Dijkstra(int s){
	dinh	V[MAX];												// Tap dinh cua do thi
	int		Ctmp[MAX][MAX];										// mang Ctmp luu mang A voi quy uoc Ctmp[i][j] = INF neu ko co canh ij
	cout << "\n Duong di ngan nhat theo Dijkstra: ";
	// Tao ma tran trong so Ctmp: Ctmp[i][j]=A[i][j] neu A[i][j] huu han - hay la co cung (i,j)
	// Ctmp[i][j]=INF neu khong co cung (i,j) - so sanh voi: A[i][j]=0 neu khong co cung (i,j)
	for(int i=1; i<=n; i++){
		for(int j=1; j<=n; j++){
			if(A[i][j]!=0){	//Co cung (i,j) hay A[i][j] huu han
				Ctmp[i][j]=A[i][j];
			}
			else{	//Khong co cung (i,j) hay A[i][j]=0
				Ctmp[i][j]=INF;
			}
		}
	}
	//Khoi tao
	d[s]=0;														// nhan dinh s = 0
	for(int i=1; i<=n; i++){									// tao tap V theo quy uoc: V[i].num=i; V[i].id=i
		V[i].num=i;	V[i].id=i;	truoc[i]=s;
	}
	//d[s]=0 da duoc co dinh, loai dinh s khoi tap V
	V[s].id=0;
	for(int i=1; i<=n; i++){									// Tu s gan nhan cac dinh v con lai, dong thoi gan truoc[v]=s
		if(V[i].id!=0){
			d[V[i].num]=Ctmp[s][V[i].num];	truoc[V[i].num]=s;
		}
	}
	
	int t=1;													// t dung de nhan biet khi nao tap T rong, moi lan co dinh nhan d[u], ta loai u khoi tap V bang cach gan: V[u].id=0
																// tap T rong khi moi phan tu cua tap V deu co: V[..].id=0
	while(t!=0){												// tap T <> rong
		int mindu=INF, u;
		for(int i=1; i<=n; i++){								// tim min du voi u thuoc tap T
			if((V[i].id!=0)&&(d[V[i].num]<mindu)){
				mindu=d[V[i].num];	u=i;
			}
		}
		V[u].id=0;												// co dinh nhan dinh u, loai u khoi T
		for(int i=1; i<=n; i++){								// Tu u gan lai nhan cac dinh v con lai, dong thoi gan truoc[v]=u
			if((V[i].id!=0)&&(d[V[i].num]>(d[V[u].num]+Ctmp[u][V[i].num]))){		// chi cap nhat nhan tot hon
				d[V[i].num]=(d[V[u].num]+Ctmp[u][V[i].num]);	truoc[V[i].num]=u;
			}
		}
		t=0;													// tam thoi coi T la rong, sau do ktra xem T thuc su rong hay ko?
		for(int i=1; i<=n; i++){
			if(V[i].id!=0){
				t=V[i].id;	break;
			}
	...
	// In ket qua khoang cach tu dinh s den cac dinh t cua do thi
	for(int t=1; t<=n; t++){
		cout << "\n K/c tu dinh " << s << " den dinh " << t << " = " << d[t];
		cout << "\n\tduong di: " << t << " <- ";					// in dinh dau tien tren duong di ngan nhat
...
// Ham tim duong di ngan nhat tu 1 dinh s den cac dinh con lai cua do thi co huong khong co chu trinh am (thuat toan Bellman-Ford)
void	Dothi::Bellman_Ford(int s){
	dinh	V[MAX];												// Tap dinh cua do thi
	int		k, u, v;											// mang nhan: mang khoang cach tu 1 dinh den cac dinh khac
	int		Ctmp[MAX][MAX];										// mang Ctmp luu mang A voi quy uoc Ctmp[i][j] = INF neu ko co canh ij
	cout << "\n Duong di ngan nhat theo Bellman-Ford: ";
	// Tao ma tran trong so Ctmp
	for(int i=1; i<=n; i++){
		for(int j=1; j<=n; j++){
			if(A[i][j]!=0){	//Co cung (i,j) hay A[i][j] huu han
				Ctmp[i][j]=A[i][j];
	...
	for(int i=1; i<=n; i++){									// tao tap V la tap cac dinh, id moi dinh = ten cua dinh
		V[i].num=i;	V[i].id=i;
	}
	// Khoi tao
	for(int i=1; i<=n; i++){									// Tu s gan nhan cac dinh khac, va gan: truoc[v]=s
		d[V[i].num]=Ctmp[s][V[i].num];	truoc[V[i].num]=s;
	}
	// Lap
	d[s]=0;
	for(k=1; k<=n-2; k++){
		for(v=1; v<=n; v++){									// xet cac dinh khac dinh s
			if(v!=s){
				for(u=1; u<=n; u++){							// xet cac cung di tu cac dinh u den dinh v
					if((Ctmp[u][v]!=INF)&&(d[u]!=INF)&&(d[v]>d[u]+Ctmp[u][v])){
						d[v]=d[u]+Ctmp[u][v];	truoc[v]=u;
					}
				}
			}
		}
...
// Ham tim duong di ngan nhat giua cac cap dinh cua do thi co huong khong co chu trinh am (su dung thuat toan Floyd)
void	Dothi::Floyd(){
	int		next[MAX][MAX];										// mang next luu duong di
	int		Ctmp[MAX][MAX];										// mang Ctmp luu mang A voi quy uoc Ctmp[i][j] = INF neu ko co canh ij
	cout << "\n Thuat toan Floyd - khoang cach giua cac cap dinh: ";
	// Tao ma tran trong so Ctmp: Ctmp[i][j]=A[i][j] neu A[i][j] huu han - hay la co cung (i,j)
	// Ctmp[i][j]=INF neu khong co cung (i,j) - so sanh voi: A[i][j]=0 neu khong co cung (i,j)
	for(int i=1; i<=n; i++){
		for(int j=1; j<=n; j++){
	...
	// Khoi tao
	for(int i=1; i<=n; i++){									// xet tung cap dinh
		for(int j=1; j<=n; j++){
			dFloyd[i][j]=Ctmp[i][j];
			if(Ctmp[i][j]!=INF){
				next[i][j]=j;
	...
	for(int k=1; k<=n; k++){
		for(int i=1; i<=n; i++){
			for(int j=1; j<=n; j++){
				if (dFloyd[i][k]!=INF && dFloyd[k][j]!=INF && dFloyd[i][j]>(dFloyd[i][k]+dFloyd[k][j])){
					dFloyd[i][j]=dFloyd[i][k]+dFloyd[k][j];
					next[i][j]=next[i][k];
				}
			}
		}
	...
	for(int i=1; i<=n; i++){
		...
		for(int j=1; j<=n; j++){
			int itmp=i;
			if(dFloyd[i][j]>=INF){	cout << "\n Khong co duong di tu dinh " << i << " den " << j;	}
			else {
				cout << "\n K/c tu dinh " << i << " den dinh " << j << " = " << dFloyd[i][j];
				cout << "\n\t Duong di: " << itmp;
				while(itmp!=j){
					cout << " --> " << next[itmp][j];
					itmp=next[itmp][j];	}
...
int main(){
	...
	//if(G.DocDuLieu("6_1_Dijkstra_n13.in")){
	//if(G.DocDuLieu("6_2_Bellman_Ford_n5.in")){
	//if(G.DocDuLieu("6_3_Floyd_n4.in")){
	if(G.DocDuLieu("6_Bai6.in")){
		//cout << "\n Doc file OK";
